...
*/
SELECT al.name AS "album name", al.year, t.name AS "track", t.length FROM track t JOIN album al ON t.album_id = al.id JOIN artist art ON al.artist_id = art.id WHERE art.id = 1 ORDER BY al.name, t.track_number;